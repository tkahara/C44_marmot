...
// 🌟【会員用】カード3項目が全て完全に揃っているかチェック
boolean isUserCardComplete = false;
if (loginUser != null) {
	String cNum = loginUser.getCardNumber();
	String cName = loginUser.getCardName();
	String cExpiry = loginUser.getCardExpiration(); // プロパティ名に合わせて調整してください
	
	if (cNum != null && !cNum.trim().isEmpty() &&
		cName != null && !cName.trim().isEmpty() &&
		cExpiry != null && !cExpiry.trim().isEmpty()) {
		isUserCardComplete = true;
	}
}
...
.text-center, .text-end { white-space: nowrap; }
.table td:first-child { word-break: break-all; white-space: normal; }
.total-amount-cell { min-width: 120px; text-align: right !important; }
</style>
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-check mb-2">
							...
							<label class="form-check-label" for="pay1"> クレジットカード (末尾: <%=loginUser.getCardNumber().substring(loginUser.getCardNumber().length() - 4)%>) </label>
						</div>
						<%
							} else {
						%>
						<div class="form-check mb-2 text-muted">
							<input class="form-check-input" type="radio" id="pay1_user_disabled" disabled> 
							<label class="form-check-label text-danger small fw-bold" for="pay1_user_disabled"> ⚠️ クレジットカード（登録情報が不完全です） </label>
						</div>
						<%
							}
						} else {
						...
						if (guestCardNum != null && !guestCardNum.trim().isEmpty()) {
							String maskedNum = (guestCardNum.length() == 16) ? guestCardNum.substring(12) : "****";
						%>
						<div class="form-check mb-2">
							<input class="form-check-input" type="radio" name="payment" id="pay1_guest" value="credit" <%="credit".equals(currentPayment) ? "checked" : ""%>> 
							<label class="form-check-label" for="pay1_guest"> クレジットカード (末尾: <%=maskedNum%>) </label>
						</div>
						<%
						} else {
						%>
						<div class="form-check mb-2 text-muted">
							<input class="form-check-input" type="radio" id="pay1_disabled" disabled> 
							<label class="form-check-label" for="pay1_disabled"> クレジットカード（カード情報未入力） </label>
						</div>
						<%
						}
						}
				...
				<table class="table align-middle">
					...
					<tbody>
						...
						<tr class="table-light fs-5">
							...
							<td class="text-end fw-bold text-danger total-amount-cell"><%=nf.format(confirmTotalAmount)%> 円</td>
						</tr>
					</tbody>
				</table>
