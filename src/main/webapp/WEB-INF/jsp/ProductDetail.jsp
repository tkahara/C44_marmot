...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/product01.css">
...
<main class="container my-5">
		...
		<div class="col-md-6">
			...
			<form action="addToCart" method="post" class="mt-4">
				...
				<div class="mb-4 d-flex align-items-center gap-2"
					...
					<label for="quantity" class="form-label mb-0 text-nowrap fw-bold"
						style="color: #4A3325;">数量：</label>

					<%-- 💡 修正：input[type=number] から 1〜10 のプルダウン（選択式）へ変更 --%>
					<select class="form-select" id="quantity" name="quantity" required
						style="border: 1px solid rgba(111, 78, 55, 0.25);">
						<%
						for (int i = 1; i <= 10; i++) {
						%>
				...
				<button type="submit" class="btn btn-lg w-100 text-white fw-bold"
					style="background-color: #6F4E37; border-radius: 6px; box-shadow: 0 2px 4px rgba(111, 78, 55, 0.2);">
					🛒 カートに追加する</button>
			</form>
		</div>
	</div>
	...
	%>
	<div class="alert alert-danger text-center">該当の商品が見つかりませんでした。</div>
	<%
	}
	%>
</main>
