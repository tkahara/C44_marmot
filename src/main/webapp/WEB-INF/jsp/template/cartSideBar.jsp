			...
			<div class="border-bottom pb-3 mb-3">
				<div class="d-flex justify-content-between align-items-center mb-2">
					<div>
						...
						<small class="text-muted">単価: <%=p.getPrice()%>円
						</small>
					</div>
				...
				<div class="d-flex justify-content-between align-items-center">
					...
					<form action="CartUpdateServlet" method="POST" class="m-0">
						<input type="hidden" name="action" value="delete"> <input
							type="hidden" name="productName" value="<%=p.getProductName()%>">
						<button type="submit"
							class="btn btn-outline-danger btn-sm py-0 px-2">キャンセル</button>
					</form>
				</div>
			</div>
			<%
		...
		<div class="mt-auto pt-3 border-top bg-white">
			...
			<div class="d-grid gap-2">
				...
				<div class="alert alert-danger small py-2 mb-2"
					style="border-radius: 8px; background-color: #FDF2F2; color: #A94442; border-color: rgba(169, 68, 66, 0.15); font-weight: 500;">
					⚠️ カートに商品が入っていません。</div>
				<button type="button" class="btn btn-secondary btn-lg" disabled
					style="opacity: 0.6; cursor: not-allowed;">購入手続きへ進む</button>

				<%-- 🌟 カートに商品が1つ以上入っている場合（通常時） --%>
				<%
				} else {
				%>
